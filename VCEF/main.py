...
from thumos_features import ThumosFeature

from model import WTALModel
...
class Trainer():
    ...
    def calculate_pesudo_target(self, batch_size, label, topk_indices, topk_values):
        cls_agnostic_gt = []
        ...
        for b in range(batch_size):
            ...
            topk_values_b = topk_values[b, :, label_indices_b]  # topk, num_actions
            cls_agnostic_gt_b = torch.zeros((1, 1, self.config.num_segments)).cuda()

            # positive examples
            for gt_i in range(len(label_indices_b)):
                high_conf_indices = topk_indices_b[:, gt_i][topk_values_b[:, gt_i] > confidence_threshold]
                cls_agnostic_gt_b[0, 0, high_conf_indices] = 1
            ...
            if mean_ap > self.best_mAP:
                ...
                with open(f_path, 'w') as f:
                    ...
                    for item in list(zip(iou, ap)):
                        sum = sum + item[1]
                        count += 1
                        string_to_write = 'Detection map @ %0.2f = %0.2f' % (item[0], item[1] * 100)
                        f.write(string_to_write + '\n')
                        f.flush()
        ...
        for epoch in range(self.config.num_epochs):
            ...
            for _data, _label, _, _, _ in self.train_loader:
                ...
                combined_cas = misc_utils.instance_selection_function(
                    ...
                    att_rgb.permute(0, 2, 1))  # P^V

                topk_values, topk_indices = torch.topk(combined_cas, self.config.num_segments // 8, dim=1)
                ...
                att_gt = self.calculate_pesudo_target(batch_size, _label, topk_indices, topk_values)
